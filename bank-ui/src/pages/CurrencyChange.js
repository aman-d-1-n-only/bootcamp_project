...
export default function CurrencyChange() {
  ...
  useEffect(() => {
    ...
    axios
      .post(`http://data.fixer.io/api/symbols?access_key=${API_KEY}`)
      .then((res) => {
  ...
  const convertCurrency = () => {
    ...
    axios
      .post(
        `http://data.fixer.io/api/latest?access_key=${API_KEY}&base =${currency.from}`
      )
      .then((res) => {
        ...
        const temp = `${currency.amount} ${currency.from} = ${
          currency.amount *
          (res.data.rates[currency.to] / res.data.rates[currency.from])
        } ${currency.to}`;
          ...
          <CardBody className="px-10">
            <form className="flex flex-col gap-y-6">
              ...
              <div className=" w-full flex  items-end justify-around">
                ...
                <ArrowsRightLeftIcon
                  className="h-8 w-8 mb-1 p-1 cursor-pointer hover:scale-110 rounded-lg hover:shadow hover:bg-gray-50"
                  onClick={handleSwap}
                />
                {/* <Select
      id="to"
